...
def loop(request, application):
    lp = asyncqtpy.QEventLoop(application)
...
@pytest.fixture(
    params=[None, asyncqtpy.QThreadExecutor, ThreadPoolExecutor, ProcessPoolExecutor],
)
...
def test_can_function_as_context_manager(application):
    ...
    with asyncqtpy.QEventLoop(application) as loop:
        assert isinstance(loop, asyncqtpy.QEventLoop)
